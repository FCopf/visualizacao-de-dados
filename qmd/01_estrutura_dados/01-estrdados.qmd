...
Considere a tabela abaixo, construída a partir do livro de Biocenoses em Reservatórios: padrões espaciais e temporais [@rodriguesetal2005].
...
A tabela é formada por `r nrow(res)` linhas referentes a reservatórios do estado do Paraná e `r ncol(res)` colunas em que constam informações sobre cada reservatório, sendo elas:
...
No exemplo referentes aos reservatórios do Paraná, cada unidade amostral (linhas da tabela) é um reservatório. Os reservatórios são descritos por meio das variáveis dispostas nas colunas da tabela. O reservatório de `r res$Reservatorio[i]` por exemplo faz parte da bacia do `r res$Bacia[i]`, foi formado no ano de `r res$Fechamento[i]`, tem área de `r res$Area[i]` $km^2$, pH igual a `r res$pH[i]` e assim por diante.
