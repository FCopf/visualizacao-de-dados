...
Inicialmente, vamos retomar o conceito de **variância amostral** (@sec-variacao).
...
No @sec-variacao representamos esta quantia simplesmente por $s^2$. Aqui vamos usar uma notação diferente ($s^2_Y$), pois haverá outros estimadores de variância envolvidos, de modo que deveremos ser mais claros a respeito de qual estimador estaremos nos referindo.
...
Importe a base de dados [ctigre_haddon.csv](https://github.com/FCopf/probest-cambientais/blob/master/datasets/ctigre_haddon.csv){target="_blank"}:

```{r}
#| echo: false
#| eval: true
tigre = read_delim('datasets/ctigre_haddon.csv')
...
#| echo: true
#| eval: false
tigre = read_delim('ctigre_haddon.csv')
tigre %>% 
  flextable() %>% 
  set_header_labels(
    values = list(
